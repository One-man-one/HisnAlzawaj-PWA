...
(function () {
  ...
  function request(path, options) {
    ...
    return fetch(window.HISN.API + path, {
      ...
      // ✅ **تجديدُ الجلسة المنزلق**: الوسيط يعيد توكناً جديداً مع الردّ الناجح
      // متى مضت على الحاليّ ساعة — فالنشطُ لا يُخرَج بعد يومٍ من دخوله
      // (الشرح عند `renewal_for` في مستودع البوت). ⚠️ ويُحفظ فقط إن كان
      // صاحبه ما زال داخلاً: ردٌّ وصل بعد «خروج» لا يعيد الجلسة.
      var fresh = res.headers.get('X-Session-Token');
      if (fresh && readToken()) writeToken(fresh);
      if (res.status === 401) {
